    ...
    def _forward_box(self, features, proposals, targets=None, task="ObjectDet", replace_pred_boxes_with_gt_proposals=False):
        ...
        if self.training:
            ...
            if replace_pred_boxes_with_gt_proposals is True:
                ...
                pred_instances = [
                    Instances(image_sizes[0], pred_boxes=Boxes(input_boxes), scores=proposals[0].objectness_logits, pred_classes=torch.zeros(len(input_boxes), dtype=torch.int64).to(device))
                ]
            else:
